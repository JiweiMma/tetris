...
public class myPanel extends JPanel implements KeyListener, Runnable
{
	...
	int nextNum; //下一块的编号
	int fallCycle = 400; //每fallCycle单位时间下降一格
	...
	{ //检查可否继续移动 dx表示判断x方向 dy同理
		for(int i = 0; i < 4; i++)
		{
			int x = (getx(i)-15)/30+dx;
			int y = (gety(i)-10)/30+dy;
			if(x < 0 || x == W || y == H || cell[y][x] != -1)
			{ //如果无法移动
